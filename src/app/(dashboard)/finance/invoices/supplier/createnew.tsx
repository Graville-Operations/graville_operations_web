    ...
    >
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          >
            <span className="text-xs flex-shrink-0" style={{ color: 'var(--gv-text-muted)' }}>Particulars</span>
...
export default function CreateInvoiceModal({ onClose, onSuccess }: CreateInvoiceModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    console.debug('[CreateInvoiceModal] Submitting payload:', payload);

    try {
      setSubmitting(true);
      const { data } = await api.post('/invoices/create', payload);
      console.debug('[CreateInvoiceModal] Create response:', data);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error('[CreateInvoiceModal] Create error:', err?.response ?? err);
      const detail =
