...
export default function TaskDetailPage() {
  ...
  if (!Number.isFinite(taskId)) {
    return (
      <div className="gv-page-dashboard flex items-center justify-center text-sm text-(--gv-text-subtle)">
        Invalid task.
      </div>
    );
  }
  ...
  return (
    <div className="gv-page-dashboard">
      <div className="gv-nav sticky top-0 z-20 px-4 sm:px-6 flex items-center gap-3 flex-wrap">
        <button onClick={() => router.back()} className="gv-btn-outline p-2 w-9 h-9 rounded-xl shrink-0">
          <ArrowLeft size={18} />
        </button>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-6 space-y-6">
        ...
        {task && (
          <div className="gv-card p-5 space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="flex items-center gap-1.5 text-xs text-(--gv-text-subtle)">
                <CalendarRange size={12} className="text-(--gv-brand)" />
                {formatDate(task.start_date)} → {formatDate(task.end_date)}
              </span>
            </div>

            {task.description && (
              <p className="text-sm text-(--gv-text-muted) leading-relaxed">
                {task.description}
              </p>
            )}
          </div>
        ...
        <div>
          ...
          {retryInfo && (
            <div className="gv-card flex items-center gap-3 text-sm text-(--gv-text-muted) p-3 mb-2">
          ...
          {subsError && subtasks.length === 0 && (
            <div className="gv-card flex items-center gap-3 text-sm text-red-400 border-red-500/20 bg-red-500/10 p-3 mb-2">
              <AlertCircle size={16} className="shrink-0" /> {subsError}
              <button onClick={loadSubtasks} className="ml-auto underline underline-offset-2 hover:text-red-300 shrink-0">
                Retry
              </button>
            </div>
          )}
          ...
          {!loadingSubs && !subsError && subtasks.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-(--gv-text-subtle)">
              <Layers size={36} strokeWidth={1} />
              <p className="text-sm">No subtasks yet</p>
              <button
                onClick={goToCreateSubtask}
                className="text-sm text-(--gv-brand) hover:text-(--gv-brand-hover) underline underline-offset-2"
              >
                Add first subtask
              </button>
            </div>
              ...
              {subtasks.map((sub) => {
                ...
                return (
                  <div key={sub.id} className="gv-card w-full text-left flex items-center gap-4 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-(--gv-text-primary) truncate">{sub.name}</p>
                      {sub.description && (
                        <p className="text-xs text-(--gv-text-subtle) truncate mt-0.5">{sub.description}</p>
                      )}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <ChevronRight size={14} className="text-(--gv-text-faint)" />
                    </div>
                  </div>
                );
              })}
