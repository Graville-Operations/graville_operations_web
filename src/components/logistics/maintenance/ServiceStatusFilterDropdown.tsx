...
import { VehicleServiceStatus, SERVICE_STATUS_META } from '@/types/vehicle-service';

const STATUS_OPTIONS = Object.values(VehicleServiceStatus).map((value) => ({
  value,
  label: SERVICE_STATUS_META[value].label,
}));

interface ServiceStatusFilterDropdownProps {
  value: VehicleServiceStatus | null;
  onChange: (status: VehicleServiceStatus | null) => void;
}

export default function ServiceStatusFilterDropdown({ value, onChange }: ServiceStatusFilterDropdownProps) {
