...
import { InitiateDeliveryForm, emptyInitiateDeliveryForm } from '@/types/material-delivery';
import DriverSelectDropdown from './DriverSelectDropdown';

interface InitiateDeliveryOverlayProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (form: InitiateDeliveryForm) => void;
}
...
export default function InitiateDeliveryOverlay({ open, onClose, onSubmit }: InitiateDeliveryOverlayProps) {
        ...
        >
          <div className="flex items-start justify-between p-6 border-b border-[color:var(--border)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-[color:var(--foreground)]">Initiate Delivery</h2>
                <p className="text-xs text-[color:var(--muted-foreground)] mt-0.5">Log a new internal material delivery</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
            <Field label="Destination" required>
              <input className={inputCls} placeholder="e.g. Mishi Mboko Site" value={form.destination}
                onChange={(e) => setForm((p) => ({ ...p, destination: e.target.value }))} />
            </Field>
            <Field label="Assign Driver">
              <DriverSelectDropdown
                value={form.driver}
                onChange={(driver) => setForm((p) => ({ ...p, driver }))}
              />
            </Field>
            <Field label="Purpose / Description">
              <textarea rows={3} className={`${inputCls} resize-none`} placeholder="What this delivery is for…" value={form.purpose}
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              >
                ...
                {submitting ? 'Saving…' : 'Initiate Delivery'}
              </button>
