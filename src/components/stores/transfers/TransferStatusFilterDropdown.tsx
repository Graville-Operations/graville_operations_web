...
import { TransferStatus, TRANSFER_STATUS_META } from '@/types/transfer';

const STATUS_OPTIONS = Object.values(TransferStatus).map((value) => ({
  value,
  label: TRANSFER_STATUS_META[value].label,
}));

interface TransferStatusFilterDropdownProps {
  value: TransferStatus | null;
  onChange: (status: TransferStatus | null) => void;
}

export default function TransferStatusFilterDropdown({
  value,
  onChange,
}: TransferStatusFilterDropdownProps) {
      ...
      {open && (
        ...
        >
          <button
            ...
            style={{
              color: !value ? '#33907c' : 'var(--gv-text-muted)',
              background: !value ? 'rgba(51,144,124,0.1)' : 'transparent',
            }}
          >
            All Statuses
            {!value && <Check size={13} />}
